...
for(n in 1:m){
  ...
  cat("\n\n")
  cat(gsub("\n", "\n\n", kstructures[kstructures$model == model.names[[n]],]$structure))
  cat("\n\n")
  ## find fold with lavaan object for the model
  for(f in 1:k){
  if(model.names[[n]] %in% names(kfa$cfas[[f]])) break  
}
    do.call(semPlot::semPaths, args = c(list(object = kfa$cfas[[f]][[n]],
