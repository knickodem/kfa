...
rel.map <- data.frame(col_keys = names(krels$reliabilities[[1]]),
                      top = c("factor", rep(c("omega_h", "alpha"), each = 3)),
                      bottom = c("factor", rep(c("mean", "range", "flag"), times = 2)))
...
for(n in 1:m){
  ...
  rel.flex <- flextable::colformat_double(rel.flex, j = c(2, 5), digits = digits)
